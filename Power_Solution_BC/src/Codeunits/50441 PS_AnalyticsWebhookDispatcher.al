codeunit 50441 "PS_Analytics Webhook Dispatcher"
{
    ...
    local procedure ResourceOnAfterInsert(var Rec: Record Resource)
    begin
        TriggerSync('recursos');
    end;

    [EventSubscriber(ObjectType::Table, Database::Resource, 'OnAfterModifyEvent', '', false, false)]
    local procedure ResourceOnAfterModify(var Rec: Record Resource)
    begin
        TriggerSync('recursos');
    end;
    ...
    local procedure JLEOnAfterInsert(var Rec: Record "Job Ledger Entry")
    begin
    ...
    local procedure JLEOnAfterModify(var Rec: Record "Job Ledger Entry")
    begin
        TriggerSync('movimientos_proyectos');
    end;

    [EventSubscriber(ObjectType::Table, Database::"Job Ledger Entry", 'OnAfterDeleteEvent', '', false, false)]
    local procedure JLEOnAfterDelete(var Rec: Record "Job Ledger Entry")
    begin
        TriggerSync('movimientos_proyectos');
    end;
    ...
    local procedure DimValOnAfterInsert(var Rec: Record "Dimension Value")
    begin
    ...
    local procedure DimValOnAfterModify(var Rec: Record "Dimension Value")
    begin
        TriggerSync('centros_de_responsabilidad');
        TriggerSync('tecnologias');
        TriggerSync('tipologias');
    ...
    local procedure DimValOnAfterDelete(var Rec: Record "Dimension Value")
    begin
        TriggerSync('centros_de_responsabilidad');
        TriggerSync('tecnologias');
        TriggerSync('tipologias');
    ...
    local procedure PSYearOnAfterInsert(var Rec: Record "PS_Year")
    begin
        TriggerSync('ps_year');
    end;

    [EventSubscriber(ObjectType::Table, Database::"PS_Year", 'OnAfterModifyEvent', '', false, false)]
    local procedure PSYearOnAfterModify(var Rec: Record "PS_Year")
    begin
        TriggerSync('ps_year');
    end;

    [EventSubscriber(ObjectType::Table, Database::"PS_Year", 'OnAfterDeleteEvent', '', false, false)]
    local procedure PSYearOnAfterDelete(var Rec: Record "PS_Year")
    begin
        TriggerSync('ps_year');
    end;
