    ...
    const loadCompleteDays = async () => {
      ...
      try {
        ...
        if (headerError) {
          ...
          setErrorCompleteDays("Error obteniendo header para el período");
          return;
        }

        // Si no hay header, continuar con 0 líneas (no es un error en dashboard)
        let timesheetLines = [];
        if (headerData?.id) {
          const { data: fetchedLines, error: timesheetError } =
            await supabaseClient
              .from("timesheet")
  ...
  useEffect(() => {
    ...
    async function loadPendingHours() {
      try {
        ...
        // 1) Obtener recurso (código y tipo de calendario)
        ...
        const resourceNo = resourceData.code;
        let calendarCode = resourceData.calendar_type;
        let headerId = null;

        // 2) Intentar obtener header del período actual para calendar_code efectivo
        const { data: headerData, error: headerError } = await supabaseClient
          .from("resource_timesheet_header")
        ...
        if (headerError) {
          ...
          setErrorHours("Error obteniendo header del período");
          return;
        }
        if (headerData?.id) {
          headerId = headerData.id;
          calendarCode = headerData.resource_calendar || calendarCode;
        }

        // 3) Rango de fechas del mes a partir de allocationPeriod MYY-MMM
        ...
        // 4) Obtener días de calendario para horas requeridas
        ...
        const requiredSum = Object.values(dailyRequired).reduce((a, b) => a + (Number(b) || 0), 0);

        // 5) Sumar horas imputadas del período si existe header
        let imputedSum = 0;
        if (headerId) {
          ...
          imputedSum = Object.values(imputedByDay).reduce((a, b) => a + (Number(b) || 0), 0);
        }

        const pending = Math.max(0, requiredSum - imputedSum);
        if (!cancelled) setPendingHours(pending);
      } catch (e) {
  ...
  return (
    ...
    >
      ...
      <section
        ...
        style={{ flex: 1, minHeight: 0, overflowY: "visible" }}
      >
        <article
